...
import { useRouter } from "next/navigation";
import { toast } from "sonner";
import { AddProductDialog } from "@/components/wholesaler/addproduct-dialog";
...
export function SellerDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [invRes, orderRes] = await Promise.all([
        ...
        fetch(`${API}/seller/pending-orders`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      ...
      setPendingOrders(Array.isArray(orderData) ? orderData : []);
    } catch (e) {
      console.error(e);
      toast.error("Failed to load dashboard");
    } finally {
  ...
  const markShipped = async (orderId: string) => {
    const id = toast.loading("Marking order as shipped...");
    ...
    try {
      ...
      setPendingOrders((p) => p.filter((o) => o.id !== orderId));
      toast.success("Order shipped", { id });
    } catch {
  ...
  const printQR = async (order: any) => {
    try {
      ...
      const url = `${window.location.origin}/verify-qr?data=${encodeURIComponent(
        encrypted
      )}`;
  ...
  // ---------------- DERIVED ----------------
  const lowStock = inventory.filter((i) => (i.stock ?? 0) < 5);
  ...
  return (
    <div className="min-h-screen p-10 space-y-10 bg-background">
      {/* HEADER */}
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        >
          ...
          <h3 className="text-2xl font-bold">{lowStock.length} Products</h3>
          <p className="text-muted-foreground">Low stock alert</p>
        </Card>

        <Card className="p-6">
          <h3 className="font-bold">Best Seller</h3>
          <p className="text-blue-500">Smart Fitness Band</p>
        </Card>
      </div>
                ...
                <TableRow key={o.id}>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Button
                      ...
                      onClick={() => printQR(o)}
                    >
                      <Package className="w-4 h-4 mr-1" /> QR
                    </Button>
                    <Button size="sm" onClick={() => markShipped(o.id)}>
