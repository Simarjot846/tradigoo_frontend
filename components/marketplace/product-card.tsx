...
const ProductCard = memo(
  ({ product, addToCart, getCategoryEmoji }: ProductCardProps) => {
    const router = useRouter();
    ...
    return (
      ...
      >
        ...
        <div className="h-52 p-6 bg-zinc-50 dark:bg-[#161616] flex items-center justify-center relative overflow-hidden">
          ...
          <div className="absolute top-3 left-3 z-10">
            <Badge className="bg-blue-600/90 text-white border-0 text-[10px] uppercase font-bold px-2 py-0.5 shadow-lg shadow-blue-600/20">
              Best Seller
            </Badge>
          </div>
        </div>
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          {/* ✅ PRICE FIX */}
          <div className="mt-1 flex items-baseline gap-1">
            <span className="text-sm align-top text-zinc-500 dark:text-zinc-400">
              ₹
            </span>
          ...
          <div className="mt-auto flex gap-2">
            <Button
              onClick={(e) => {
                e.stopPropagation();
                addToCart(product.id, product.minimumOrderQty || 1);
                router.push("/cart");
              }}
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold h-10 shadow-lg shadow-blue-600/20 transition-all hover:scale-[1.02]"
            >
